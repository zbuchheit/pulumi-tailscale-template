...
import * as cloudinitMod from "@pulumi/tailscale_cloudinit_config";
...
export interface TailscaleComponentArgs {
  ...
  additionalParts?: pulumi.Input<
    pulumi.Input<cloudinitMod.types.input.AdditionalPartsArgs>[]
  >;
}
...
export class TailscaleComponent extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    let tailscaleCloudinit: pulumi.Output<string>;
    const combinedTags = { ...args.tags, Name: name };

    tailscaleCloudinit = new cloudinitMod.Module(
      `${name}-ts-cloudinit`,
      {
